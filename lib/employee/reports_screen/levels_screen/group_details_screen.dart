...
class _GroupDetailsScreenState extends State<GroupDetailsScreen> {
  ...
  // ✅ الدالة المعدلة - استخدام الـ Endpoint الجديد RemoveFromGroup
  Future<void> _deleteStudent(int studentId, String studentName) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('token');

      // ✅ استخدام الـ Endpoint الجديد المخصص لحذف الطالب من المجموعة
      final url = Uri.parse(
          'https://nourelman.runasp.net/api/Student/RemoveFromGroup?StId=$studentId');

      debugPrint("🗑️ Removing student from group: $studentId");
      debugPrint("🔗 URL: $url");
      ...
      debugPrint("📥 RemoveFromGroup response (${response.statusCode}): ${response.body}");
      ...
      if (response.statusCode == 200 || response.statusCode == 204) {
        ...
        // محاولة فك الـ response لعرض رسالة الخطأ إن وجدت
        String errorMsg = "فشل الحذف: ${response.statusCode}";
        try {
    ...
    showDialog(
      ...
      builder: (context) => Directionality(
        ...
        child: AlertDialog(
          ...
          content: Text("هل أنت متأكد من حذف $studentName من المجموعة؟"),
          actions: [
            TextButton(onPressed: () => Navigator.pop(context), child: const Text("إلغاء")),
            ElevatedButton(
